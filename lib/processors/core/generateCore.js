...
import {
  generateInitialTypeset,
  generateRanges,
  generateLastRangeTypeset
} from './typography.js'
            ...
            tasks: [
              {
                ...
                callback: next => generateInitialTypeset(stylesheet.typesetRules, (err, css) => callback(err, css, next))
              },

              {
                name: `Generating Viewport Width Range Typesets`,
                callback: next => generateRanges(stylesheet.typesetRules, (err, css) => callback(err, css, next))
              },

              {
                name: `Generating Last Viewport Width Range Typeset`,
                callback: next => generateLastRangeTypeset(stylesheet.typesetRules, (err, css) => callback(err, css, next))
              }
            ]
          }).then(() => {
            core.append(root)
