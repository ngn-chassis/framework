...
export default postcss.plugin('generate-core', (tree, theme) => {
  return (root, result) => new Promise((resolve, reject) => {
    let { components, modules } = tree
    ...
    QueueUtils.queue({
      ...
      tasks: [
...

        {
          ...
          callback: next => ComponentUtils.generateCSS(components, (err, css) => {
            // console.log(css.toString());
            callback(err, css, next)
          })
        }
      ]
    })
