...
export default class Entry {
  ...
  process (cb) {
    ...
    this.#paths.forEach(filepath => {
      ...
      queue.add(`Processing`, next => this.#processStylesheet(stylesheet, err => {
        if (err) {
          return cb(err)
        }
  ...
  #processStylesheet = (stylesheet, cb) => {
    ...
    queue.add(`  Generating themed versions`, next => {
      ...
      //   return this.#resolveStylesheet(stylesheet, stylesheet.root, 'default', false, (err, output) => {
      //     if (err) {
      //       return cb(err)
      //     }
      //
  ...
  #resolveVersions = (stylesheet, versions, cb) => {
    ...
    if (!versions.some(version => version.theme === 'default')) {
      versions.push({ theme: 'default' })
    }

    versions.forEach(version => {
      queue.add(`Resolving "${version}" version`, next => {
        this.#resolveStylesheet(stylesheet, root.clone(), version.theme, true, (err, output) => {
          if (err) {
            return cb(err)
          }
  ...
  #resolveStylesheet = (stylesheet, root, theme, appendThemeName, cb) => {
    let { filepath } = stylesheet
    theme = NGN.coalesce(stylesheet.getTheme(theme), DefaultTheme)
