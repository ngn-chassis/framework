...
import initializeTypographyEngine from './plugins/typography/initializeTypographyEngine.js'
import resolveComponents from './plugins/components/resolveComponents.js'
...
export default class Entry {
  ...
  #generateThemedVersion = (version, cb) => {
    ...
    if (!Config.typography.disabled) {
      plugins.push(initializeTypographyEngine(this.#stylesheet.typesetRules))
    }

    let components = this.#stylesheet.components.collated

    if (Object.keys(components).length > 0) {
      plugins.push(resolveComponents(components, theme))
    }
