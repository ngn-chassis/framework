...
function generateDecls (properties) {
  return Object.keys(properties).map(property => {
    return CSSUtils.createDecl(`--${property}`, properties[property])
  })
}

export default postcss.plugin('chassis-custom-properties', (theme, includeInternal = false) => {
  return (root, result) => new Promise((resolve, reject) => {
    let { width, gutter } = CONFIG.layout
    let properties = Object.keys(theme.properties)
    let rule = CSSUtils.createRule(':root')

    rule.append(generateDecls(theme.properties))
    ...
    if (properties.length > 0 || includeInternal) {
      root.prepend(
        parser.parse(
          `/* Custom Properties ***********************************************************/

${rule.toString()}`,
          { from: 'chassis.custom-properties' }
        )
      )
    }
