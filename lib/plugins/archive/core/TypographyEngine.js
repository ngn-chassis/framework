...
export default class TypographyEngine {
  ...
  static initialize (theme, applications, cb) {
    ...
    QueueUtils.run({
      ...
      tasks: [{
        name: 'Generating Setting Media Queries',
        callback: next => {
          this.#registerSettings(applications)
          next()
        }
      }, {
  ...
  static #registerSetting = (application, min = null, max = null) => {
    let indexes = {
    ...
    if (!application.bounds.min) {
      this.#initialSettings.push(application)
    }
  ...
  static #registerSettings = applications => {
    ...
    applications.forEach(application => {
      ...
      if (!min || !max) {
        ...
        switch (viewport) {
          case min: return this.#registerSetting(application, min)
          case max: return this.#registerSetting(application, null, max)
        }
      }
      ...
      this.#orphanSettings.push(application)
    })
  }

  static #renderSetting = (root, application, width, size, columns) => {
  ...
  static #renderInitial = cb => {
    ...
    ;[...this.#applications, ...this.#initialSettings].forEach(application => {
      this.#renderSetting(root, application, width.min, baseFontSize, 1)
    })
    ...
    this.#viewports.forEach((viewport, i) => {
      ...
      if (viewport.type !== 'range') {
        ...
        return viewport.applications.forEach(application => {
          this.#renderSetting(query, application, viewport.bounds.min, fontSize, columns)
        })
      }
