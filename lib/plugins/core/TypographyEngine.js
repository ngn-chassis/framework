...
export default class TypographyEngine {
  ...
  static #registerApplications = applications => {
    ...
    applications.forEach(application => {
      ...
      // TODO: Add an additional private property (object) to this class mapping this application to its associated viewports
      // Add a property to the object specifying buffer.
      // Then, when rendering viewports, first generate a list of all queries needed,
      // one for each viewport and one for any intermediate queries needed for applications which do not
      // conform to the bounds of a single viewport

      // Example- If neiter the min or max bound of the application conforms to the min or max bound of a viewport,
      // don't specify font-size or line-height inside a viewport's media query. Instead, use a standalone media query/ies
      // for the application.

      // If min and max viewports are the same, add application to that viewport
      // If they are different, add to each, but specify that additional queries may be needed to handle buffer

      if (!min || !max) {
        let viewport = NGN.coalesce(min, max)
        console.log(`ONLY MATCHED --${viewport.name}.${min ? 'min' : 'max'} - only render at relevant viewports`)
      ...
      if (min.name === max.name) {
        console.log('MATCH - render only at the matched viewport')
