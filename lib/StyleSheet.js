...
export default class Stylesheet {
  ...
  #validateComponents = cb => {
    ...
    this.#validate('Validating Component', this.#components, (component, next) => {
      ...
      let components = this.isImported ? [...this.parent.components, ...this.components] : this.components

      extensions.forEach(extension => {
        let matches = components.filter(component => component.name === extension.superclass)

        if (matches.length === 0) {
          return cb(extension.error(`\nCannot extend non-existent component "${extension.superclass}"`, { word: extension.superclass }))
        }

        let component = matches[0]
        extension = new ComponentExtension(component, extension)
