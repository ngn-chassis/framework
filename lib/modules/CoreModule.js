...
export default class CoreModule extends Module {
  static render (stylesheet, theme, cb) {
    ...
    root.append(this.customProperties(theme))
    ...
    this.components(stylesheet.components, (err, result) => {
      if (err) {
        return cb(err)
      }
      ...
      this.resolve(`chassis.${theme.name}`, root, cb)
    })
  }
  ...
  static components (components, cb) {
    let root = CSSUtils.createRoot([])
    ...
    queue.on('complete', () => cb(null, root.toString()))
