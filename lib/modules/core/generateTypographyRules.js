...
export default function generateTypographyRules (cb) {
  let { smoothScaling, minFontSize, maxFontSize, scaleRatio } = Config.typography
  ...
  filteredRanges.forEach((range, index) => {
    ...
    if (fontSize < minFontSize) {
      return cb([
        error,
        `Specified font size (${fontSize}) is smaller than the minimum font size (${minFontSize})`
      ])
    }
    ...
    if (fontSize > maxFontSize) {
      return cb([
        ...
        `Specified font size (${fontSize}) is larger than the maximum font size (${maxFontSize})`
      ])
    }
