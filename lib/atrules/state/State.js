...
export default class State extends Class {
  ...
  // Maybe put state-specific code here and override this getter on the
  // Component class
  get selector () {
    let { selector } = this.#source

    if (!selector) {
      selector = this.parent.getStateSelector(this.name)
    }
  ...
  getStateSelector (name) {
    let state = this.states.find(state => state.name === name)
