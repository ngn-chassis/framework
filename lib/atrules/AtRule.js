...
export default class AtRule {
  ...
  #validateInput = (spec, input, index) => {
    ...
    if (!!spec.reserved && input.value !== spec.reserved) {
      throw this.error(`\n${err.arg(spec.name, index)}: Expected "${reserved}" but received "${input.value}" \n${this.#formatError}`, {
  ...
  #processArg = (params, spec, input, index) => {
    ...
    if (index !== null && spec.chainable) {
      this.#chainable = true
      chained = params.slice(index + 1)
    }

    let arg = new Arg(spec.name, input, this.#offset)

    if (chained) {
      return [arg, ...chained.filter((arg, i) => {
