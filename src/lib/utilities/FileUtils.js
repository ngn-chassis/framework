...
export default class FileUtils {
  ...
  static parseStyleSheet (filepath) {
    return parser.parse(fs.readFileSync(filepath), { from: filepath })
  }
  ...
  static parseStyleSheets (filepaths, relative = true) {
    const output = this.parseStyleSheet(filepaths[0], relative)
    const remainingFilepaths = filepaths.slice(1)
    remainingFilepaths.forEach(path => output.append(this.parseStyleSheet(path, relative)))
    return output
  }
